...
(function($) {
    ...
    $.fn.autocolumnlist = function(params){
        var options = $.extend({}, defaults, params), content = this.html(), elClass = this.attr('class');
        this.replaceWith('<div class="'+elClass+' g-column">'+content+'</div>');
        return $('.g-column').each(function() {
            ...
            if (dimension > 0) {
                ...
                for (i=0; i<options.columns; i++) {
                    if((dimension - start) < options.min){
                        start = start - 1;
                    }
